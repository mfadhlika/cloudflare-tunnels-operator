...
#[tokio::test]
async fn test_ingress_controller_existing_records() {
    env_logger::init();
    ...
    let hostname = "whoami.example.com";
    let cname_record = "e2e-test.cfargotunnel.com";
    let txt_record = "heritage=cloudflare-tunnels-operator,cloudflare-tunnels-operator/owner=homelab,cloudflare-tunnels-operator/resource=ingress/default/whoami";
    ...
    let list_dns_initial_mock = setup_list_dns_mock(
        &mut server,
        zone_id,
        hostname,
        vec![
            ...
            (
                hostname.to_string(),
                "TXT".to_string(),
                "heritage=cloudflare-tunnels-operator,cloudflare-tunnels-operator/owner=cloud,cloudflare-tunnels-operator/resource=ingress/default/whoami".to_string(),
                "023e105f4ecef8ad9ca31a8372d0c355".to_string(),
            ),
        ],
    )
    .await;

    let list_dns_current_mock = setup_list_dns_mock(
        &mut server,
        zone_id,
        hostname,
        vec![
    ...
    list_tunnel_mock.expect_at_least(1).assert_async().await;
    list_dns_initial_mock.assert_async().await;
    list_dns_current_mock
        .expect_at_least(2)
        .assert_async()
        .await;
    create_txt_mock.assert_async().await;
